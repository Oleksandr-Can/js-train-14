...
function getDayOfWeek(date) {
  ...
  return {inputDate, dayOfWeek: day};
}
...
function getDaysInMonth(date) {
  ...
  const lastDayNextMonth = (Date.parse(newDate) - (1000 * 60 * 60 * 24));
  
  // Отримання числа останнього дня поточного місяця - це кількість днів у місяці.
  const daysInMonth = new Date(lastDayNextMonth).getDate();
...
function getAge(birthDate) {
  ...
  return {birthDate: inputDate, age}
}
...
function compareDates(date1, date2) {
  ...
  if (typeof date1.getTime !== 'function' && typeof date1.getTime !== 'function') {
...
function getDaysDifference(startDate, endDate) {
  ...
  const ensdMilliseconds = endDate.getTime();
  // Різниця в мілісекундах між двома датами.
  const difference = ensdMilliseconds -startMilliseconds; 
